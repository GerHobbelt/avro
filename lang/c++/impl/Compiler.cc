...
struct Field {
    ...
    Field(string n, vector<string> a, NodePtr v, GenericDatum dv)
        : name(std::move(n)), aliases(std::move(a)), schema(std::move(v)), defaultValue(std::move(dv)) {}
};
...
static Field makeField(const Entity &e, SymbolTable &st, const string &ns) {
    ...
    return Field(std::move(n), std::move(aliases), node, d);
}
...
static NodePtr makeRecordNode(const Entity &e, const Name &name,
    ...
    if (doc == nullptr) {
        ...
        node = new NodeRecord(asSingleAttribute(name), asSingleAttribute(*doc),
                              fieldValues, fieldNames, fieldAliases, defaultValues);
    }
    return NodePtr(node);
}
