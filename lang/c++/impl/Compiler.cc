...
struct Field {
    ...
    Field(string n, vector<string> a, NodePtr v, GenericDatum dv, const CustomFields& cf)
        : name(std::move(n)), aliases(std::move(a)), schema(std::move(v)), defaultValue(std::move(dv)), customFields(cf) {}
};
